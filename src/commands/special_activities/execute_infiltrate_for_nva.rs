...
pub fn execute_infiltrate_for_nva(
    infiltrate_instructions: &[InfiltrateInstructions],
    map: &mut Map,
    track: &mut Track,
    ...
    for entry in infiltrate_instructions.iter() {
        if entry.get_type_of_infiltrate() == "place" {
            let nva_troops_to_place = track.get_trail()
                + map
                    .get_space(entry.get_space())
                    .unwrap()
            ...
            let previous_number_of_nva_troops_in_space = map
                .get_space(entry.get_space())
                .unwrap()
                .get_number_of_nva_troops();

            map.get_space_mut(entry.get_space())
                .unwrap()
                .set_number_of_nva_troops(
                    previous_number_of_nva_troops_in_space + nva_troops_to_place,
                )?;
        } else if entry.get_type_of_infiltrate() == "exchange" {
            let current_amount_of_nva_guerrillas = map
                .get_space_mut(entry.get_space())
                .unwrap()
                .get_number_of_underground_nva_guerrillas();

            map.get_space_mut(entry.get_space())
                .unwrap()
                .set_number_of_underground_nva_guerrillas(0);
            ...
            map.get_space_mut(entry.get_space())
                .unwrap()
                .set_number_of_nva_troops(
                    previous_number_of_nva_troops_in_space + current_amount_of_nva_guerrillas,
                )?;
        } else if entry.get_type_of_infiltrate() == "replace" {
            // Sanity check: there must be a vc piece specified.

            if entry.get_vc_piece().is_none() {
            ...
            if let Some(vc_piece) = entry.get_vc_piece() {
                if vc_piece == "underground_vc_guerrilla" {
                    ...
                    let previous_number_of_underground_vc_guerrillas = map
                        .get_space(entry.get_space())
                        .unwrap()
                        .get_number_of_underground_vc_guerrillas();
                    ...
                    map.get_space_mut(entry.get_space())
                        .unwrap()
                        .set_number_of_underground_vc_guerrillas(
                            previous_number_of_underground_vc_guerrillas - 1,
                        );

                    let previous_number_of_nva_guerrillas = map
                        .get_space(entry.get_space())
                        .unwrap()
                        .get_number_of_underground_nva_guerrillas();
                    ...
                    map.get_space_mut(entry.get_space())
                        .unwrap()
                        .set_number_of_underground_nva_guerrillas(
                            previous_number_of_nva_guerrillas + 1,
                        );
                    ...
                    panic!(
                        "Replace not contemplated for vc piece: {:?}",
                        entry.get_vc_piece()
                    );
                }
            }
