...
pub fn register_infiltrate_instructions(
    ...
    if split_typed_input_commands[1] == "place" || split_typed_input_commands[1] == "exchange" {
        interpreted_intentions.add_infiltrate_instructions(InfiltrateInstructions::new(
            split_typed_input_commands[1].to_string(),
            transform_typed_space_into_space_identifier(split_typed_input_commands[2]),
            None,
        ));
        ...
        interpreted_intentions.add_infiltrate_instructions(InfiltrateInstructions::new(
            split_typed_input_commands[1].to_string(),
            transform_typed_space_into_space_identifier(split_typed_input_commands[2]),
            Some(split_typed_input_commands[3].to_string()),
        ));
    } else {
