...
pub struct InformationComposer {
    information_tag: String,
}

impl Default for InformationComposer {
    fn default() -> Self {
        Self::new()
    }
}

impl<'a> InformationComposer {
    pub fn new() -> InformationComposer {
        InformationComposer {
            information_tag: "  (*)".to_string(),
        }
    }
    ...
    pub fn compose(&self, text: &str, buffer: &'a mut Buffer) -> Result<&'a mut Buffer, String> {
        ...
        if let Err(error) = write!(buffer, "{}", self.information_tag) {
            return Err(error.to_string());
        }
