...
impl AvailableForces {
    ...
    pub fn remove_amount_of_us_irregulars(&mut self, amount: u8) -> Result<u8, String> {
        // Will have to check edge cases, like not being able to remove the amount provided.
        let negative_amount: i8 = produce_negative_number_from_u8(amount);

        // Will have to store the amount of us irregulars available before removing, because those will be all
        // that we will be able to add to the location, if that happens.
        let previous_amount_of_us_irregulars_available = self.us_irregulars_available;

        self.us_irregulars_available = sum_i8_to_u8(negative_amount, self.us_irregulars_available);
