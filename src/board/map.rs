...
impl Map {
    ...
    pub fn get_space(&self, space_to_retrieve: SpaceIdentifiers) -> Result<&Spaces, String> {
        ...
        match retrieved_space {
            ...
            None => return Err(format!("Couldn't find the corresponding space for {:?}. That should be impossible.", space_to_retrieve))
        }
    }
    ...
    pub fn add_space(&mut self, space_to_add: Spaces) -> Result<(), String> {
        ...
        if self.spaces.contains_key(&space_to_add.get_space_identifier()){
...
mod tests {
    ...
    #[test]
    fn test_after_adding_two_different_spaces_to_the_hashmap_there_should_be_two_entries_on_the_hashmap() -> Result<(), String>{
